...
Alchemist~\cite{alchemist} è un simulatore di sistemi complessi (ad esempio sistemi
altamente distribuiti, biologici o pervasivi), scritto in Java e
...
sia per uno studio in cui gli stessi algoritmi vengono applicati
...
\subsection{Il motore a eventi discreti}
...
\subsection{Il \emph{layer} come funzione della posizione}
...
Le molecole e le concentrazioni descrivono lo stato interno dei singoli nodi. Alcune
grandezze, come la temperatura o la concentrazione di un inquinante, non appartengono
ad alcun nodo ma all'ambiente: un nodo che si sposta ne osserva valori diversi perché
cambia la sua posizione, non il suo stato.
...
valore di tipo \texttt{T}. Il \Cref{lst:layer-interface} ne mostra l'interfaccia.

\lstinputlisting[
    language=Java,
    label={lst:layer-interface},
    caption={L'interfaccia del Layer}
]{listings/Layer.java}
...
gaussiana bidimensionale centrata su un punto.

\iffalse
Queste realizzazioni sono adeguate a
...
Alchemist rappresenta l'istante corrente di una simulazione come un valore di tipo
\texttt{Time}, la cui interfaccia espone il tempo come un numero in virgola mobile a doppia
precisione ed è privo di un'unità di misura intrinseca: la sua interpretazione è una
convenzione lasciata al modellista della simulazione (ad esempio secondi, ore,
giorni), non un vincolo imposto dal simulatore; il suo valore parte da zero e cresce
...
reale: un \texttt{OSMEnvironment} può caricare estratti di OpenStreetMap~\cite{osm} e collocare i
...
\iffalse
È questa asimmetria che fa nascere il problema affrontato in questa tesi:
...
\subsection{Il caricamento da YAML e i vincoli sui costruttori}
...
con cui costruirlo. Questo meccanismo, basato sulla reflection, ha condizionato la forma di
ogni classe destinata a essere istanziata da YAML.

Il layer gaussiano introdotto nella Sezione~\ref{subsec:alchemist-layer} è costruito
da un costruttore di sei parametri (\Cref{lst:bidimensional-gaussian-layer-kotlin}), 
di cui il primo (\texttt{baseline}) e l'ultimo
(\texttt{sigmaY}) sono opzionali.

\lstinputlisting[
    language=Kotlin,
    label={lst:bidimensional-gaussian-layer-kotlin},
    caption={Costruttore di BidimensionalGaussianLayer}
]{listings/BidimensionalGaussianLayer.kt}
...
forma più diretta, mostrata dal \Cref{lst:bidimensional-gaussian-layer}, i parametri sono specificati posizionalmente.

\lstinputlisting[
    language=YAML,
    label={lst:bidimensional-gaussian-layer},
    caption={Costruzione posizionale di BidimensionalGaussianLayer}
]{listings/bidimensional.yml}
...
scritta direttamente in Kotlin, sarebbe quella mostrata dal \Cref{lst:bidimensional-gaussian-equivalent}.

\lstinputlisting[
    language=Kotlin,
    label={lst:bidimensional-gaussian-equivalent},
    caption={Costruzione equivalente in Kotlin di BidimensionalGaussianLayer}
]{listings/bidimensional-equivalent.kt}
...
nome dell'argomento a cui si riferiscono, come mostrato dal \Cref{lst:bidimensional-gaussian-layer-named}.

\lstinputlisting[
    language=YAML,
    label={lst:bidimensional-gaussian-layer-named},
    caption={Costruzione esplicita di BidimensionalGaussianLayer}
]{listings/bidimensional-names.yml}
...
Un vincolo riguarda i tipi generici. La dichiarazione presente nel file YAML indica
la classe da istanziare, ma non i parametri di tipo con cui specializzarla.
Una classe generica non è pertanto istanziabile da
configurazione, e il suo utilizzo richiede l'esistenza di una specializzazione concreta,
ovvero di una sottoclasse in cui il parametro di tipo sia già fissato.
...
\section{Copernicus e i \emph{data store} ECMWF}
\label{sec:copernicus}

Copernicus è il programma di osservazione della Terra dell'Unione Europea, articolato in
servizi tematici. 
%Fra questi, il \ac{CEMS} %\footnote{\url{https://emergency.copernicus.eu/}}
...
infrastruttura software, differenziati solo per host e per il dominio dei dataset che
espongono, quindi un unico client è sufficiente a interrogarli tutti. Essi sono:

\begin{itemize}
    \item \ac{CDS}, data store del \ac{C3S}, che ospita fra gli altri ERA5, la
    rianalisi climatica globale prodotta da \ac{ECMWF};
    \item \ac{EWDS}, data store del \ac{CEMS}, che ospita i dataset di \ac{EFAS} e
    ...
    \item \ac{ADS}, data store del \ac{CAMS}, relativo a composizione atmosferica,
...
lo stesso adottato dal client Python ufficiale di \ac{ECMWF}\footnote{Il client ufficiale ECMWF per il recupero dai data store: \url{https://github.com/ecmwf/ecmwf-datastores-client}.}.

\iffalse
I tre data store espongono i propri dataset in due modalità. La prima è l'interfaccia web
di cui ciascuno dispone: selezionati i parametri di un dataset, un pulsante avvia lo
scaricamento. La seconda è programmatica: la stessa interfaccia, a selezione avvenuta,
mostra il frammento di codice da fornire al client Python ufficiale, che effettua la
richiesta per conto dell'utente. La selezione dei parametri è quindi la stessa nei due
casi, e a distinguerli è solo il modo in cui viene trasmessa.
\fi

Ciascun data store espone i dataset tramite un'interfaccia web che, selezionati i parametri,
ne consente lo scaricamento oppure mostra il frammento di codice equivalente per il client
Python ufficiale: la selezione dei parametri è quindi la stessa nei due casi.

Il risultato di una richiesta non è necessariamente un solo file. Una selezione può
individuare più prodotti, ad esempio più variabili o più intervalli temporali, e in tal
caso i data store consegnano un archivio compresso anziché un file singolo. Alcuni dataset
espongono fra i propri parametri la forma desiderata (archivio o singolo file), ma non si tratta di una facoltà
generale: quale delle due forme venga consegnata dipende dal dataset e dalla selezione
effettuata. Il formato dei file contenuti, fra quelli descritti nella
Sezione~\ref{sec:formati-raster}, è invece scelto dall'utente.

\section{Formati e convenzioni dei dati raster}
...
\item \ac{GRIB}, distribuito dai data store nelle versioni GRIB1 e GRIB2, è uno standard
    binario definito dalla \ac{WMO}~\cite{wmo306}, con l'obiettivo di
...
\item \ac{NetCDF}, distribuito dai data store nelle versioni NetCDF3 e NetCDF4, è un formato
    ...
    multidimensionali~\cite{netcdffile}. A differenza di \ac{GRIB}, un file \ac{NetCDF} è
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/netcdf-file-structure.png}
    \caption[Struttura CF di un file NetCDF]{Struttura CF di un file NetCDF.}
    \label{fig:netcdf-structure}
\end{figure}

Un aspetto comune ai due formati riguarda la rappresentazione dei valori mancanti, che nei
dataset forniti dai data store non è un caso raro: le misure della temperatura a due metri dal suolo,
ad esempio, sono definite solo in corrispondenza di tratti terreni, e lasciano prive di valore
tutte le celle in corrispondenza di acqua.
...
Lo schema è descritto, a livello di standard, dall'\ac{OGC} in \emph{OGC API - Processes -
...
vita di un processo si articola in tre fasi:
...
sottomissione; e il recupero dei risultati, disponibile una volta che il processo ha raggiunto
uno stato terminale. Lo standard definisce inoltre l'insieme degli stati che un processo può
...
esempio \texttt{status} per lo stato di un processo, e consente alle implementazioni di

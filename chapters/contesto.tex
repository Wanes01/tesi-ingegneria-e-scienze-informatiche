...
suoi data store, i formati e le convenzioni con cui i dati geospaziali raster vengono distribuiti,
e il paradigma su cui si basano le richieste asincrone verso i data store.
...
Un \texttt{Layer<T, P>} è, nel modello di Alchemist, una funzione totale che associa a
...
disponibile in nelle fasi iniziali del ciclo di vita di una simulazione. 
...
Che i tre condividono la stessa infrastruttura, per cui un unico
...
di una grandezza fisica su assi di latitudine, longitudine e tempo, dove l'asse temporale
è una successione di istanti assoluti riferiti ad un calendario di riferimento.

\begin{itemize}
\item \ac{GRIB}, distribuito dai data store nelle versioni GRIB1 e GRIB2, è uno standard
    binario definito dalla \ac{WMO}~\cite{wmo_manual_codes}, con l'obiettivo di
    ...
    temperatura registrata a due metri dal suolo, ma la corrispondenza fra codice e
...
\item \ac{NetCDF}, distribuito dai data store nelle versioni NetCDF3 e NetCDF4, è un formato
    ...
    dagli store di riferimento, è la convenzione \ac{CF}~\cite{cf_conventions}.
\end{itemize}
...
le altre celle della griglia. In un file \ac{NetCDF} conforme alle convenzioni \ac{CF}
l'assenza di un valore è segnalata dall'attributo \texttt{\_FillValue} o \texttt{missing\_value}:
un valore riservato, che le librerie di lettura riconoscono come indicatore di dato mancante.
...
discusse nel Capitolo~\ref{chap:implementazione}, riguarda la spaziatura degli
assi: le varie convenzioni richiedono che latitudine e longitudine siano
strettamente monotone, ma non che siano equispaziate.

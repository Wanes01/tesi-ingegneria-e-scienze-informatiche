...
\section{Architettura e scelte trasversali}
\label{sec:architettura-generale}

\subsection{La suddivisione in quattro sottomoduli}
...
Il Layer è l'unico punto in cui le diverse parti si incontrano (\req{NF2}).

\subsection{Il tipo comune dei campioni}
\label{subsec:tipo-campioni}

Bisogna decidere quale tipo attribuire ai
campioni durante l'intero percorso che va dalla lettura del file al valore restituito
dal layer.

I formati \ac{GRIB} e \ac{NetCDF} esposti dai data store Copernicus espongono
tutte le misure in virgola mobile, anche quando in origine sono
...
La scelta non è dettata solo dalla capacità rappresentativa:
il meccanismo di caricamento non è in grado di istanziare classi generiche (Sezione~\ref{subsec:alchemist-loader});
ciò comporta che rendere generica anche la rappresentazione,
introducendo una griglia parametrica in un tipo \texttt{T} arbitrario, avrebbe
richiesto una specializzazione lungo l'intera gerarchia
...
misura (\req{F7}). Le alternative principali
...
\req{NF7}. In secondo luogo, \texttt{NaN} si propaga automaticamente attraverso le
...
\section{Rappresentazione di griglie e serie temporali}
...
\subsection{La memorizzazione densa o sparsa delle griglie}
...
della griglia; la stessa grandezza campionata su tratti costieri è invece definita
solo lungo i tratti terreni, lasciando priva di misura la maggior parte della griglia.

La distinzione tra le due è invisibile a chi usa la griglia:
...
\subsection{Il caricamento anticipato della serie}
...
risponde a \req{NF7}: spostando l'intero accesso al file system nella fase di costruzione,
...
La stessa scelta soddisfa \req{NF6}, poiché un file illeggibile o incoerente con gli
...
La costruzione di una serie richiede anche di indicare quale variabile estrarre, poiché un file
può contenerne più di una (\Cref{subsec:variabile-impl}).
\iffalse
...
Capitolo~\ref{chap:implementazione}.
\fi
...
\fi
Come stabilito da \req{F5}, la regola impiegata per la produzione del valore
del layer non può essere unica. Questa sezione ne descrive la realizzazione come
famiglia di strategie~\cite{gof1994} e la estende alla traduzione del valore nel tipo
esposto ai nodi, richiesta da \req{F6}.

\subsection{Interpolazioni separabili e non separabili}
\label{subsec:interpolazione}

\iffalse
Il requisito si traduce nell'introduzione di un'astrazione per la regola e di
un insieme di realizzazioni intercambiabili, secondo lo schema noto come \emph{strategia}~\cite{gof1994}.
\fi
...
il peso che colloca l'istante richiesto fra le due, e ne produce un valore. La ragione per
...
Per poter realizzare queste regole in modo riutilizzabile, il
...
\fi
Le strategie decidono anche il trattamento dei valori mancanti: \texttt{BilinearInterpolation},
ad esempio, propaga \texttt{NaN} se manca almeno uno dei quattro campioni, e
\texttt{LinearInterpolation} fa lo stesso se manca uno dei due valori risolti.
...
\texttt{MissingValueFallbackConverter<T>}, un decoratore~\cite{gof1994} che riceve un altro convertitore e un valore di
...
\section{Il layer come punto di composizione}
\label{sec:layer}
...
\subsection{Il tempo letto a ogni interrogazione}
...
\fi
Come stabilito nella \Cref{subsec:corrispondenza-temporale}, la corrispondenza fra tempo
di simulazione e istanti del mondo reale è confinata nel componente che ne ha bisogno: il layer.
...
dichiarati da chi configura la simulazione (\req{F3}) e restano
...
seconda.
\iffalse
: alla costruzione del layer l'asse temporale della serie viene ricondotto al
...
dell'intervallo coperto dall'asse così ottenuto, come discusso nella \Cref{subsec:estensione-spaziale}.
Se l'istante richiesto precede
...
\subsection{La gerarchia e il flusso di un'interrogazione}
...
\iffalse
Il layer non contiene alcuna logica di
...
Il layer legge il tempo di simulazione dall'environment (\Cref{subsec:layer-pull-based}) e individua,
sull'asse temporale della serie già espresso in tempo di simulazione, i due campioni che
lo racchiudono e le griglie corrispondenti. Verifica poi che la posizione ricada nella
regione coperta, respingendo la richiesta in caso contrario (\Cref{subsec:estensione-spaziale}, \req{NF5}), calcola
il peso temporale e delega alla strategia di interpolazione la produzione del valore (\req{F4})
e al convertitore la sua traduzione nel tipo esposto (\req{F6}). Nessuno di questi passi accede
a risorse esterne (\req{NF7}).

\FloatBarrier

\subsection{La costruzione da serie, cartella o data store}
...
fornendo direttamente una serie, senza leggere file né accedere alla rete (\req{NF3}).
...
In tutte e tre le modalità, l'acquisizione e la lettura dei dati
...
alla simulazione di disporre dei dati di cui ha bisogno, come \req{NF6} richiede.
\fi
In tutte le modalità, l'acquisizione e lettura dei dati avvengono durante
la costruzione del layer, ed è quindi lì che si manifestano condizioni
che possono impedire alla simulazione di disporre dei dati necessari (\req{NF6}).
...
esistenza soddisfa \req{F11}, rendendo dichiarabile una specializzazione del layer
nella configurazione YAML (Sezione~\ref{subsec:alchemist-loader}).
La specializzazione è inoltre il punto in cui i valori predefiniti delle
...
\subsection{I contratti di chiave, fonte e gestore della cache}
...
\end{itemize}
\fi
Il sottosistema è costituito dalle tre entità di acquisizione del modello del
dominio (\Cref{sec:modello-dominio}), precisandone il contratto: una \texttt{CacheKey}
deve sapersi tradurre deterministicamente in un nome di cartella; un
\texttt{ExternalDataProvider<R>} deposita i file in una cartella indicata dal chiamante,
senza vincoli sul modo o sul formato; un \texttt{CacheManager<R>} restituisce la cartella
corrispondente a una descrizione.

Fonte e gestore della cache sono parametrici nel tipo della descrizione, vincolato a essere una
\texttt{CacheKey}. Il parametro è ciò che garantisce staticamente che
questi componenti accettino la stessa descrizione, senza
...
provengano da un data store Copernicus o da altre sorgenti.
Le uniche realizzazioni che conoscono i data store Copernicus
...
In questo modo eventuali fonti diverse possono essere aggiunte
implementando solo la coppia \texttt{CacheKey} ed \texttt{ExternalDataProvider<R>},
senza modificare il gestore della cache (\req{NF2}).

\subsection{L'identità di una richiesta}
\label{subsec:acquisizione-identita}

Il requisito \req{NF4} chiede che la stessa richiesta corrisponda in modo deterministico

...
\section{La lettura dei file NetCDF e GRIB}
...
memoria: le convenzioni \ac{CF} raccomandano l'ordine
...
\subsection{La fusione dei file in un'unica serie}
...
\section{Il supporto comune alle strategie}
\label{sec:strategie-impl}

\subsection{La ricerca binaria e l'estrapolazione costante}
...
La funzione si appoggia ad \texttt{Arrays.binarySearch}, che restituisce l'indice della
...
\subsection{L'uguaglianza delle strategie senza stato}
...
\section{La lettura e la costruzione del layer}
\label{sec:layer-impl}

\subsection{La conversione degli istanti della serie}
...
\subsection{La risoluzione di un valore per una posizione}
\label{subsec:interrogazione-impl}

La risoluzione segue la sequenza della \Cref{fig:layer-sequenza}, e il
...
\texttt{null} se il layer viene interrogato prima. In tal caso si assume lo zero,
...
\subsection{I costruttori e i vincoli del caricamento}
...
8601~\cite{iso8601}, compatto e convertibile senza ambiguità.
...
benché un valore predefinito fosse naturale. Ogni argomento facoltativo genera una
...
Il \Cref{lst:yaml} mostra un esempio della forma assunta dal layer in un file di configurazione (\req{F11}).
Gli argomenti omessi assumono i valori di default.
...
\section{La cache locale e il client dei data store}
\label{sec:acquisizione-impl}

\subsection{Dalla richiesta al nome della cartella di cache}
...
costruzione è la stessa per selezioni equivalenti ma con diverso ordine.
...
\subsection{La produzione atomica delle voci di cache}
...
e la popola tramite la fonte con \texttt{provider.fetch}.
...
precedono con un prefisso proprio, assieme all'indirizzo del data store
...
al posto di interrompere l'attesa. Il modulo ripartisce gli stati noti in tre categorie:
...
digest calcolato localmente ne ha sempre 32. Un confronto diretto fra le due stringhe
...
\subsection{La ricostruzione della causa di un fallimento}
...
La prima istruzione realizza l'ordine di precedenza:
...
di ricondurre l'errore alla richiesta specifica.

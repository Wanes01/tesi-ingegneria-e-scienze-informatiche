...
automaticamente attraverso le operazioni aritmetiche: una regola di risoluzione che
combini campioni fra i quali ve ne sia uno mancante produce a sua volta un valore
mancante, senza che il caso debba essere trattato esplicitamente in ciascuna regola.

Il dato mancante attraversa dunque il modulo come \texttt{NaN} fino al
convertitore che decide cosa esporre (Sezione~\ref{subsec:conversione}).
...
rispettivamente una implementazione per griglie dense ed una
...
possono essere riportate solo in tratti specifici, rendendo la
griglia sparsa.
...
\texttt{RasterGrid} in memoria in fase di costruzione, la quale avviene a partire da una directory, e non da
...
domini: riceve le due griglie adiacenti all'istante richiesto, la posizione geografica e
...
Essa non costituisce tuttavia una specializzazione della composizione separabile,
bensì una sua configurazione fissata, e viene realizzata per delegazione.
...
ovverò un layer che espone direttamente il valore numerico, agendo da funzione identità,
...
% DEVI CONTINUARE DA QUAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA

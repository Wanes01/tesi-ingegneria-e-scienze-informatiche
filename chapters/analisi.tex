...
\begin{itemize}
    ...
    \item sono campionati sia nello spazio sia nel tempo, mentre la
    ...
    \item non sono calcolabili localmente ma vanno ottenuti da un
...
\begin{enumerate}
    \item \textbf{Rappresentazione}: come rappresentare in memoria, indipendentemente dalla fonte, una serie temporale di griglie con
    celle possibilmente prive di misura;
    \item \textbf{Risoluzione}: come produrre un valore per una posizione e un istante di simulazione arbitrari
    a partire da campioni discreti;
    \item \textbf{Acquisizione}: come ottenere i campioni da un servizio remoto asincrono senza riscaricarli a ogni esecuzione.
\end{enumerate}
...
Ogni sotto-problema individua un sotto-obiettivo:
...
\begin{itemize}
    ...
    senza richiedere obbligatoriamente la scrittura di codice.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{NF3 (Testabilità senza rete)}: \label{req:NF3} nessun test deve dipendere
    dalla disponibilità dei data store Copernicus o dal possesso di credenziali valide.

    \item \textbf{NF4 (Riproducibilità)}: \label{req:NF4} la stessa richiesta verso un data
    store deve corrispondere in modo deterministico alla stessa voce di cache.

    \item \textbf{NF5 (Fallimento esplicito)}: \label{req:NF5} nessuna condizione anomala
    deve produrre un valore silenziosamente scorretto: deve produrre un errore con un
    messaggio diagnostico.

    \item \textbf{NF6 (Fallimento anticipato)}: \label{req:NF6} le condizioni che
    ...
    \item \textbf{NF7 (Costo dell'interrogazione)}: \label{req:NF7} il valore di un layer
    ...
    \item \textbf{NF8 (Uso concorrente della cache)}: \label{req:NF8} Alchemist consente
...
\subsection{Tempo di simulazione e tempo reale}
...
\subsection{Il valore fra campioni adiacenti}
...
$\text{lat}_{a} \in [\text{lat}_{i}, \text{lat}_{i+1}]$ e $\text{lon}_{b} \in [\text{lon}_{j}, \text{lon}_{j+1}]$, \Cref{fig:spatial-interp}.
...
La regola deve inoltre decidere come comportarsi con eventuali valori mancanti,
di cui \req{F7} chiede un trattamento esplicito: se una posizione richiesta è
...
\subsection{Richieste fuori dalla copertura dei dati}
...
richiesta è lontana dalla regione coperta; ciò contrasterebbe con \req{NF5}, che vieta la
...
quella sul dominio spaziale. Sul dominio spaziale il rifiuto segnala un'incoerenza
...
\subsection{Le ipotesi sui dataset supportati}
...
soddisfano, poiché alcuni sono campionati su griglia nativa in una proiezione cartografica.

\iffalse
: alcuni sono campionati su una griglia nativa in una proiezione cartografica,
...
\subsection{Il fallimento al caricamento della simulazione}
...
Il requisito \req{NF6} stabilisce che tutte queste condizioni si manifestino al momento del
...
tutti riguardano il modo in cui esso viene prodotto.
...
\begin{itemize}
    ...
    \item \texttt{CacheKey} (\req{F10}, \req{NF4}): la descrizione dei dati da ottenere da
    ...
    \item \texttt{CacheManager} (\req{F9}, \req{F10}, \req{NF8}): l'entità che, data una
